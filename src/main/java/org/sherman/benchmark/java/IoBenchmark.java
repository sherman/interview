...
public class IoBenchmark {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        var fastCrc = new Crc32();
        for (var i = 0; i < testData.length; i++) {
            testData[i] = '1'; // 1
            fastCrc.update(testData[i]);
        }
        var expected = Hashing.crc32c().hashBytes(testData).asInt();
        var actual = fastCrc.hash().asInt();

        Preconditions.checkState(expected == actual, "Invalid crc32");
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (crc) {
                Preconditions.checkArgument(crcOrLength == 1905533883);
            } else {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            while ((bytes = channel.read(buffer)) != -1) {
                ...
                if (crc) {
                    ...
                    buffer.flip();
                    for (var i = 0; i < BUFFER_SIZE; i += 8) {
                        var value = buffer.getLong();
                        LONG_BYTE_ARRAY_VIEW.set(resultBuffer, 0, value);
                        for (var k = 0; k < resultBuffer.length; k++) {
                            crc32.update(resultBuffer[k]);
                        }
                        /*for (int k = 7; k >= 0; k--) {
                            crc32.update((byte) (value & 0xffL));
                            value >>= 8;
                        }*/
